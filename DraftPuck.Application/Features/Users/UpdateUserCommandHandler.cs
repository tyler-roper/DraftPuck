...
public class UpdateUserCommandHandler(IDbContext dbContext, IMapper mapper, IUserRepository userRepository, IMediator mediator, IAvatarStorageService avatarService) : IRequestHandler<UpdateUserCommand, UserDto>
{
    public async Task<UserDto> Handle(UpdateUserCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            var newAvatarPath = await TryAvatarUpload(userIdToUpdate, request.AvatarData);
    ...
    private async Task<string?> TryAvatarUpload(Guid userId, string avatarData)
    {
        try
        {
            var base64 = avatarData.Split("base64,")[1];
            ...
            return await avatarService.UploadBlobAsync($"{userId}.{extension}", ms, $"image/{extension}");
        }
        catch (Exception ex)
        {
