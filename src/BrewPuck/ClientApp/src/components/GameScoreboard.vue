                                    ...
                                    <table style="width: 100%;" class="roster-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-right" style="width: 50px;">GP</th>
                                                <th class="text-right" style="width: 50px;">G</th>
                                                <th class="text-right" style="width: 50px;">A</th>
                                                <th class="text-right" style="width: 50px;">P</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr v-for="player in getRosterByTeam(team)" :key="player.id">
                                                <td class="text-right">{{ player.primaryNumber }} </td>
                                                <td>
                                                    <span class="text-stone-700" :class="{'font-weight-bold': getPickerName(player.id) !== null}">{{ player.fullName }}</span>
                                                    <a v-if="getPickerName(player.id) === null && canPickForTeam(team)" role="button" class="text-decoration-none fs-8 font-weight-bold" @click="pick(player.id)">PICK</a>
...
<script lang="ts">
    ...
    export default class GameScoreboard extends Vue {
        ...
        lobby!: Lobby;
        pickPlayer!: (args: { gamePk: number; playerId: number; lobbyMemberId: string | null }) => Promise<void>;
        ...
        async pick(playerId: number, lobbyMemberId: string | null = null) {
            ...
            await this.pickPlayer({ gamePk: this.gamePk, playerId, lobbyMemberId });
        }
        ...
        async makeBotPick(bot: LobbyMember, team: Team) {
            ...
            if (style === BotPickStyle.Best) {
                await this.pick(availablePlayers[0].id, bot.id);
                ...
                if (availableOfBest4.length) {
                    ...
                    await this.pick(playerToPick.id, bot.id);
                } else {
                    await this.pick(availablePlayers[0].id, bot.id);
                }
                ...
                await this.pick(availablePlayers[availablePlayers.length - 1].id, bot.id);
            } else if (style === BotPickStyle.Random) {
                await this.pick(availablePlayers.random().id, bot.id);
            }
        }
