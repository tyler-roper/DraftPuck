<template>
  <div
    class="d-flex flex-column align-items-center justify-content-center bg-net"
    style="min-height: 100%; width: 100%"
  >
    <div
      class="shadow overflow-hidden p-5"
      style="border-radius: 20px; background-color: rgba(0, 0, 0, 0.5)"
    >
      <div style="width: 306px">
        <div class="text-center">
          <img src="@/assets/img/logo.png" style="width: 100%" />
          <span
            class="d-block text-uppercase text-white fs-6 mt-1"
            style="letter-spacing: 1px"
            >A live hockey drinking game</span
          >
        </div>
        <template v-if="!showLobbySettings">
          <div class="d-flex mt-4 mx-n2">
            <div class="mx-2" style="width: 30%">
              ...
              <b-form-input
                v-model="code"
                placeholder="Code"
                class="fw-bold code-input px-3 py-4 text-stone-900"
                style="width: 100%"
                maxlength="4"
              ></b-form-input>
            </div>
          ...
          <div class="mt-3 d-flex">
            <button
              @click="joinLobby"
              class="d-block btn btn-primary w-100 fw-bold py-3 text-uppercase"
              :disabled="isLoading"
            >
        ...
        <template v-if="showLobbySettings">
          ...
          <div class="mt-3">
            ...
            <div class="mt-n2" v-if="settings.bots.length > 0">
              <div
                v-for="(bot, idx) in settings.bots"
                :key="idx"
                class="my-2 d-flex mx-n2 align-items-center"
              >
                ...
                <div class="px-2" style="width: 45%">
                  <b-form-select
                    v-model="bot.pickStyle"
                    :options="botPickStyles"
                  ></b-form-select>
                </div>
                <div class="ms-auto pr-2">
                  <a role="button" @click="removeBot(bot)"
            ...
            <div>
              <a
                role="button"
                v-if="settings.bots.length < maxBots"
                class="d-block text-uppercase fw-bold"
                @click="addBot"
                >Add</a
              >
            </div>
          ...
          <div class="text-center mt-3">
            <a
              role="button"
              @click="showLobbySettings = false"
              :disabled="isLoading"
              class="fw-bold d-inline-flex"
            >
              <i
                class="fi fi-rr-caret-left d-block mr-1"
        ...
        <div class="mt-5 d-flex justify-content-between">
          ...
          <div>
            <span v-if="loadedGames" class="d-block text-stone-300"
              >({{ gameCount }} games left today)</span
            >
            <span v-if="!loadedGames" class="d-block text-stone-300"
              >Fetching games...</span
            >
        ...
        <div v-if="showHelp" class="mt-3">
          <p class="m-0 p-0 lh-2">
            <strong>DRAFTPUCK</strong> is a drinking game that takes place
            during live NHL games.<br /><br />
            The rules are simple: users pick a player from each team. If your
            player scores, you make someone else drink a beer!<br /><br />
            Looking for a twist? Add some bots! Bots make their picks based on
            the "pick style" assigned to them, which can be anything from
            auto-picking the best player available, to choosing completely at
            random. If their player scores, a random user in the lobby will be
            picked to drink!
          </p>
        </div>
      </div>
      <div class="mt-4 text-center" style="opacity: 0.8">
        ...
        <a target="_blank" class="ms-2" href="https://ropersoftworks.com"
          >Roper Softworks LLC</a
        >
      </div>
    </div>
  </div>
...
export default class Home extends Vue {
  ...
  get isLoading() {
    return this.isCreatingLobby || this.isJoiningLobby;
  }
  loadedGames = false;
  gameCount = 0;
  showHelp = false;
  ...
  botPickStyles = [
    ...
    ...Object.entries(BotPickStyle)
      .filter(([value, text]: [string, string | number]) => isNaN(Number(text)))
  ...
  async created() {
    ...
    if (latestLobby != null) {
      const latestLobbyParsed: { joinCode: string; name: string } =
        JSON.parse(latestLobby);
      this.name = latestLobbyParsed.name;
      this.code = latestLobbyParsed.joinCode;
    }

    const schedule = await NHL.getSchedule(
      format(addHours(new Date(), -4), "yyyy-MM-dd")
    );
    this.gameCount = schedule.dates[0].games.filter(
      (g) =>
        ![
  ...
  getRandomBotName(): string {
    const unusedNames = BotNames.filter(
      (botName) => !this.settings.bots.map((bot) => bot.name).includes(botName)
    );
  ...
  async createLobby() {
    ...
    if (this.settings.picksPerTeam === null) {
      this.$toast.error(
        "You must set the picks per game. For infinite, choose 0."
      );
      return;
    }
    ...
    if (
      this.settings.bots.filter(
        (bot, index) => this.settings.bots.indexOf(bot) != index
      ).length > 0
    ) {
    ...
    try {
      ...
      const lobby = await LobbyService.createLobby({
        name: this.name,
        picksPerTeam: this.settings.picksPerTeam,
      });

      const botPromises = this.settings.bots.map(
        async (b) =>
          await LobbyService.joinLobbyByCode(
            lobby.joinCode,
            b.name,
            true,
            Number(b.pickStyle)
