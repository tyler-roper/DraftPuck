<template>
  <div class="d-flex overflow-hidden flex-column" style="height: 100%">
    <template v-if="!isInvalidLobby">
        ...
        <div>
          <ol class="ml-0 pl-3">
            <li>
              ...
              <p class="mt-1 mb-0">
                Have your friends join the lobby by entering your lobby code
                <span v-if="lobby"
              ...
              <p>
                ...navigate to the
                <strong><i class="fi fi-rr-users-alt"></i> Lobby</strong> view
                and click
                <strong class="text-uppercase badge badge-primary"
                  >Invite</strong
                >.
              </p>
            </li>
            ...
            <li class="pt-3">
              ...
              <p class="mt-1">
                Once picks become available for a game, click the
                <strong>Rosters<i class="fi fi-rr-caret-right"></i></strong>
                button to see a list of each team's players.
              </p>
              <p class="mt-1">
                Available players will have a
                <strong class="text-uppercase badge badge-primary">Pick</strong>
                button beside their name, whereas players with a username have
                already been chosen.
              </p>
              <p class="mt-1">
                Choose carefully! Once you've made your choice, only the lobby
                creator can undo it for you.
              </p>
              <p class="mt-1">
                <strong>Playing with bots?</strong> Bots can make their picks as
                soon as rosters become available. The lobby creator can initiate
                the bot picks by opening the <strong>ROSTERS</strong> and
                clicking
                <strong class="text-uppercase badge badge-primary"
                  >Make Bot Picks</strong
                >.
              </p>
            </li>
            ...
            <li class="pt-3">
              ...
              <p class="mt-1">
                If one of your players scores, you get to make someone drink!
                When this happens, you'll receive a notification letting you
                know you've got a drink available. On mobile, a badge will also
                appear beside the
                <strong class="text-uppercase badge bg-stone-900 text-stone-0"
                  >Lobby</strong
                >
                button.
              </p>
              <p class="mt-1">
                Once you've earned a drink, you can dish it out by going to the
                <strong>Lobby</strong> view, clicking the desired recipient's
                name, and selecting <strong>"Give a drink!"</strong>.
              </p>
              <p class="mt-1">
                The recipient will receive a notification letting them know that
                you've sent a drink their way.
              </p>
              <p class="mt-1">
                Each player's number of drinks <i>pending</i>, drinks
                <i>given</i>, and drinks <i>taken</i>, are all listed next to
                their name in the lobby view.
              </p>
            </li>
            ...
            <li class="pt-3">
              ...
              <p class="mt-1">
                Last but certainly not least: <strong>🚨 The Feed</strong> will
                be your one-stop-shop for all lobby events, from penalties and
                goals, to drinks given and drinks taken.
              </p>
              <p class="mt-1">
                You can customize which events you want to see in your feed by
                clicking the
                <strong><i class="fi fi-sr-settings"></i> Settings</strong> icon
                at the top. You can also see a history of all drinks assigned by
                clicking the
                <strong><i class="fi fi-sr-list"></i> Drink Timeline</strong>
                button.
              </p>
            </li>
          </ol>
      ...
      <div
        class="bg-stone-900 px-sm-4 px-2 py-2 shadow position-relative d-flex align-items-center"
        style="z-index: 10"
      >
        <router-link
          to="/"
          class="banner-logo text-stone-0 text-decoration-none"
        ...
        <b-link
          class="d-flex ml-auto pt-1 text-stone-0 font-weight-bold text-decoration-none"
          role="button"
          @click="showInstructions = true"
        >
          <i class="fi fi-rr-question-square d-block ml-4"></i>
          <span class="text-uppercase ml-2 d-block" style="margin-top: -2px"
            >How To Play</span
          >
        </b-link>
      </div>
      ...
      <div class="d-flex flex-grow-1 overflow-hidden bg-stone-800">
        <template v-if="!isLoading">
          <FullScoreboard
            class="full-scoreboard flex-grow-1"
            :eventBus="eventBus"
            :class="{ 'hide-mobile': !isGameView }"
            :games="games"
            style="overflow: auto"
          ></FullScoreboard>
          ...
          <div
            ...
            :class="{ 'hide-mobile': !isFeedView && !isLobbyView }"
            style="width: 400px"
          >
            <LobbyOverview
              ref="overview"
        ...
        >
          <div class="mx-auto d-flex flex-column align-items-center">
            ...
            ></b-spinner>
            <span
              class="text-center d-block mx-auto mt-3 fs-2 text-uppercase font-weight-bold"
              >{{ loadingMessage }}...</span
            >
          </div>
        </div>
      ...
      <div
        class="bottom-nav d-flex d-sm-none bg-stone-900 shadow font-weight-bold"
        v-if="!isLoading"
      >
        <a
          role="button"
          class="text-center p-2 text-white"
          :class="{ active: isLobbyView }"
          @click="setView('lobby')"
        >
          <i v-if="pendingDrinkCount <= 0" class="fi fi-rr-users-alt"></i>
          <span v-if="pendingDrinkCount > 0" class="drink-badge"
            >🚨 {{ pendingDrinkCount }}</span
          >
          <br />
          <span>LOBBY</span>
        </a>
    ...
    <template v-if="isInvalidLobby">
      <div style="width: 100%; height: 100%" class="d-flex align-items-center">
        <div class="mx-auto d-flex flex-column align-items-center">
          <span
            class="text-center d-block mx-auto mt-3 fs-2 text-uppercase font-weight-bold"
            >Sorry, this lobby is invalid.</span
          >
        </div>
      </div>
    </template>
    ...
    <div
      v-if="currentDrink"
      class="drink-animation d-flex align-items-center justify-content-center"
    >
      <span class="text-white text-center">
        <span class="text-uppercase font-weight-bold" style="font-size: 100px"
          >Drink!</span
        >
...
export default class LobbyComponent extends Vue {
  ...
  async created() {
    try {
      ...
      this.selectedDate = format(
        addHours(this.lobby.created, -4),
        "yyyy-MM-dd"
      );

      const currentLobbyMember = this.lobby.members.find(
      ...
      localStorage.setItem(
        "latestLobby",
        JSON.stringify({ joinCode: this.lobby.joinCode, name })
      );
      ...
      this.mappedLobbyEvents = this.lobbyEvents.map((evt) =>
        this.replaceTemplatedStrings(evt)
      );
  ...
  async setGames() {
    ...
    const gamePromises = schedule.dates[0].games.map(
      async (game) => await this.getGameData(game.gamePk)
    );
  ...
  async updateGame(game: LiveGame) {
    ...
    ].includes(game.gameData.status.statusCode);
    ...
    if (Array.isArray(result)) {
      ...
      patchCollections.forEach((patchCollection) =>
        jsonpatch.applyPatch(
          game,
          patchCollection.diff as readonly jsonpatch.Operation[]
        )
      );
    } else {
      this.$set(this.games, gameIndex, await this.getGameData(game.gamePk));
  ...
  async pollForUpdates(game: LiveGame) {
    ...
    const interval = isInProgress ? 10000 : 60000;
  ...
  gameIsStale(game: LiveGame) {
    const isOver = [
      ...
      GameStatusCode.GameOver,
      GameStatusCode.Final2,
    ].includes(game.gameData.status.statusCode);
    return (
      isOver &&
      game.gameData.datetime.endDateTime &&
      game.gameData.datetime.endDateTime <= addMinutes(new Date(), -10)
    );
  }
  ...
  notifyOfDrink(lobbyEvent: LobbyEvent) {
    ...
    if (this.pendingDrinks.length === 1) this.handleDrinkAnimationQueue();
  }

  handleDrinkAnimationQueue() {
    if (this.pendingDrinks.length === 0) return;
    this.currentDrink = this.pendingDrinks[0];
    this.sendNotification("🍺 Drink!", {
      body: `Courtesy of ${this.getSenderNameByLobbyEvent(this.currentDrink)}`,
    });
  ...
  getSenderNameByLobbyEvent(lobbyEvent: LobbyEvent): string | null {
    ...
    return (
      this.lobby.members.find((m) => m.id === lobbyEvent.lobbyMemberId)?.name ??
      null
    );
  }

  async dispatchLobbyEvent(lobbyEvent: LobbyEvent) {
    const currentMember = this.lobby.members.find(
      (m) => m.userId === this.currentUserId
    );
    if (
      lobbyEvent.lobbyEventType != LobbyEventType.NewPick ||
      lobbyEvent.lobbyMemberId !== currentMember.id
    ) {
      await this.getLobby(this.joinCode);
    ...
    if (
      lobbyEvent.lobbyEventType === LobbyEventType.UserRemoved &&
      lobbyEvent.lobbyMemberId === currentMember.id
    ) {
    ...
    const templates = [
      {
        ...
        fill: (string: string, text: string): string => {
          const name =
            this.lobby.members.find((m) => m.id === lobbyEvent.lobbyMemberId)
              ?.name ?? "(name)";
      ...
      },
      {
        ...
        fill: (string: string, text: string): string => {
          const name =
            this.lobby.members.find((m) => m.id === lobbyEvent.lobbyMember2Id)
              ?.name ?? "(recipient)";
          return text.replace(string, `<strong>${name}</strong>`);
        },
      },
      {
        ...
        fill: (string: string, text: string): string => {
          const teams = this.games.find((g) => g.gamePk === lobbyEvent.gamePk)
          ...
          const player =
            ...
            ].find((p) => p.person.id === lobbyEvent.playerId)?.person
              .fullName ?? "(player)";
        ...
        fill: (string: string, text: string): string => {
          ...
          const player =
            [
              ...Object.values(teams.away.players),
              ...Object.values(teams.home.players),
            ].find((p) => p.person.id === lobbyEvent.player2Id)?.person
              .fullName ?? "(player)";
          return text.replace(string, `<strong>${player}</strong>`);
        },
      },
      {
        ...
        fill: (string: string, text: string): string => {
          ...
          const team =
            lobbyEvent.teamId === teams.home.team.id ? teams.home : teams.away;
          ...
          const playerLastName = Object.values(team.players)
            .find((p) => p.person.id === lobbyEvent.playerId)
            ?.person.fullName.split(" ")[1];
          return text.replace(
            string,
            `<span class='d-inline-block pl-3 ml-1 badge text-uppercase text-shadow' style='align-self: center; background-color: ${teamColor} !important;'>${img} ${playerLastName}</span>`
          );
        },
      },
    ];
  ...
  parseDates(lobbyEvent: LobbyEvent) {
    if (
      lobbyEvent === null ||
      lobbyEvent === undefined ||
      typeof lobbyEvent !== "object"
    )
      return lobbyEvent;
  ...
  notifyCurrentUserOfCorrectPick(lobbyEvent: LobbyEvent) {
    this.$toast.success(
      `<span class='fs-3'><strong class= text-uppercase'>Nailed it!</strong> You've been awarded a drink!</span>`,
      { duration: 5000 }
    );
    if (this.notificationPermissionsGranted) {
      const players = this.games.flatMap((g) =>
        Object.values(g.gameData.players)
      );
      const player = players.find((p) => p.id === lobbyEvent.playerId);

      if (player) {
        this.sendNotification("🚨 Give out a drink!", {
          body: `${player.fullName} shoots and scores!`,
        });
      } else {
        this.sendNotification("🚨 Give out a drink!");
      }
  ...
  get feedItems() {
    ...
    const gameItems = this.games.flatMap((game) =>
      game.liveData.plays.allPlays.reduce((items: Array<FeedItem>, play) => {
        const includedInFilters = desiredEventTypes.includes(
          play.result.eventTypeId
        );
        const happenedAfterLobbyStarted =
          play.about.dateTime >= this.lobby.created;
        const isShootoutGoal =
          play.result.eventTypeId === EventType.Goal &&
          play.about.periodType === "SHOOTOUT";
        if (includedInFilters && happenedAfterLobbyStarted && !isShootoutGoal)
          return [
            ...items,
            FeedItem.fromPlay(game.gamePk, game.liveData.linescore.teams, play),
          ];
        else return items;
      }, [])
    );

    const lobbyItems = this.mappedLobbyEvents.map((evt) =>
      FeedItem.fromLobbyEvent(evt)
    );
  ...
  get goals() {
    const goals = this.games.flatMap((game) =>
  ...
  get pendingDrinkCount() {
    return (
      this.currentLobbyMember?.picks.flatMap((p) =>
        p.drinks.filter((d) => d.recipientLobbyMemberId === null)
      ).length ?? 0
    );
  }
