...
const achievementList = computed<Array<UserAchievement>>(() => {
  return allAchievements.value.map((a) => ({
    achievementId: a.id,
...
<template>
  <ProfileSubsectionLayout title="Achievements" :show-save="false">
    ...
    <div v-else class="p-3">
      <div v-for="achievement in achievementList" :key="achievement.achievementId" class="rounded-md bg-gradient mb-3 d-flex overflow-hidden border border-stone-500 shadow" :class="{'o-50': !achievement.dateEarned}">
        ...
        <div class="py-2 px-3 flex-grow-1 d-flex flex-column justify-content-center">
          <span class="d-block fw-bold text-uppercase fs-5 ls-3" :class="{' text-stone-400' : !achievement.dateEarned }">{{ achievement.friendlyName }}</span>
