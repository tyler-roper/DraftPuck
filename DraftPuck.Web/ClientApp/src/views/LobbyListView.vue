...
const lobbySummaries = ref<UserLobbySummary[]>([])
...
async function leave(lobby: UserLobbySummary) {
...
onMounted(async () => {
  ...
  try {
    ...
    lobbySummaries.value = (await UserService.getLobbies(currentUser.value.id)).sort((a, b) => compareAsc(b.created, a.created))
  } catch (e) {
