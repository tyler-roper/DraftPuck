...
namespace Draftpuck.Nhl.Services
{
    public class NhlApiService : INhlApiService
    {
        ...
        public async Task<NhlSchedule> GetScheduleAsync(DateTime date)
            => await _httpClient.GetFromJsonAsync<NhlSchedule>($"schedule/{date:yyyy-MM-dd}");

        public async Task<NhlGameLanding> GetGameLandingAsync(int gameId)
            => await _httpClient.GetFromJsonAsync<NhlGameLanding>($"gamecenter/{gameId}/landing");

        public async Task<NhlGamePlayByPlay> GetPlayByPlayAsync(int gameId)
            => await _httpClient.GetFromJsonAsync<NhlGamePlayByPlay>($"gamecenter/{gameId}/play-by-play");
        ...
        public async Task<NhlPlayer> GetPlayerAsync(int playerId)
            => await _httpClient.GetFromJsonAsync<NhlPlayer>($"player/{playerId}/landing");
    }
}
